...
contract("thetaboard creator manager", async accounts => {
    it("Should Deploy NFT contract for direct sell", async () => {
        const manager = await ThetaboardCreatorManager.deployed();
        const thetaboardSell = await thetaboard_sell.deployed();
        const price = 10;
        const artistWallet = accounts[3];
        const deployedNFTs = await manager.deployNFTandSell(
            "my_nft",
            "https://url1",
            [accounts[1], accounts[2], thetaboardSell.address],
            thetaboardSell.address,
            price,
            0,
            0,
            artistWallet,
        ...
        const nftAddress = deployedNFTs.logs["0"].args.nftContract;
        const sell = await thetaboardSell.getNftSell(nftAddress);
        await thetaboardSell.purchaseToken(nftAddress, {
            from: accounts[0],
            value: price
        })
        assert.equal(sell.price, price);
        assert.equal(sell.artistWallet, artistWallet);
    });

    it("Should Deploy NFT contract for auction sell", async () => {
        const manager = await ThetaboardCreatorManager.deployed();
        const thetaboardAuction = await thetaboard_auction.deployed();
        const price = 10;
        const artistWallet = accounts[3];
        const deployedNFTs = await manager.deployNFTandAuction(
            "my_nft",
            "https://url1",
            [accounts[1], accounts[2], thetaboard_auction.address],
            thetaboardAuction.address,
            price,
            0,
            0,
            artistWallet,
        ...
        const nftAddress = deployedNFTs.logs["0"].args.nftContract;
        const sell = await thetaboardAuction.getNftAuction(nftAddress);
        assert.equal(sell.minBid, price);
        assert.equal(sell.artistWallet, artistWallet);
    });
})
